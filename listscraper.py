...
class LBlistscraper:
    ...
    Attributes:
        ...
        verbose_off(bool):      Turn off tqdm loading bars read from optional '-vo' flag. Default is False.
    ...
    def scrape_all_and_writeout(self, list_objs, max_workers=4):
        ...
        if self.concat == False:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers) as executor:
                _ = [executor.submit(listobj.scrape_and_write, self.output_path, self.verbose_off, self.concat) for listobj in list_objs]

        # Waits for all lists to finish before writing out
        elif self.concat == True:
            
            with concurrent.futures.ThreadPoolExecutor(max_workers) as executor:
                _ = [executor.submit(listobj.scrape, self.verbose_off, self.concat) for listobj in list_objs]
...
if __name__ == "__main__":
    ...
    LBscraper = LBlistscraper(args.listURL, args.pages, args.output_name, args.output_path, args.file, args.concat, args.verbose_off, args.threads)
